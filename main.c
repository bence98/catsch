...
int main(int argc, char *argv[])
{
	...
	for (int i = optind; i < argc; i++) {
		...
		if (err) {
			pr_err("Error while parsing input list: %s\n", strerror(err));
			return err;
		}
	}
	...
	if (err) {
		pr_err("Unexpected error: %s\n", strerror(err));
	}

	int err2 = flist_delete(l);
	if (err2) {
		pr_err("Error while closing input: %s\n", strerror(err));
		if (!err)
			err = err2;
	}
	...
	if (err2) {
		pr_err("Error while closing RNG: %s\n", strerror(err));
		if (!err)
			err = err2;
	}
